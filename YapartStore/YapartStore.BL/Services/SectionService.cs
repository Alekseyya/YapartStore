...
namespace YapartStore.BL.Services
{
    public class SectionService : ISectionService
    {
        ...
        public void DeleteItem(string sectionName)
        {
            try
            {
                var findSection = _unitOfWork.SectionRepository.GetAll().FirstOrDefault(gr => gr.Name == sectionName);
                if (findSection != null)
                {
                    _unitOfWork.GroupRepository.Delete(findSection.Id);
                }
        ...
        public IList<SectionDTO> GetAll()
        {
            try
            {
                var sections = Mapper.Map<IQueryable<Section>, IList<SectionDTO>>(_unitOfWork.SectionRepository.GetAll());
                if (sections != null)
                    return sections;
                else
                    return null;
